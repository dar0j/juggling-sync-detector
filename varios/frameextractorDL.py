...
for nBalls in [3, 4, 5, 6]:
    input_dir = f"../../new nov/{nBalls}b gifs" #Datasets/{nBalls}b
    #output_dir = f"60 fps/{nBalls}b frames 60" #30/60 fps/{nBalls}b frames 30/60
    patternscsvfolder = f"../../new nov/{nBalls}b gifs csv 30 128" # 30/60 64/128
    #os.makedirs(output_dir, exist_ok=True)
    os.makedirs(patternscsvfolder, exist_ok=True)

    video_files = glob.glob(os.path.join(input_dir, "*.gif")) # gif o mp4
    ...
    for video_path in video_files:
        ...
        count = 0
        frame_idx = 0 # descomentar al samplear a menos fps
        annotations = []
        while success:
            if frame_idx % 2 == 0: # skippear/alternar frames para que tome de un video de 60 fps muestras en 30 fps
                ...
                balls_and_hands = grid_model.predict(resized) # Predice posiciones
                row = [] # Guarda anotaciones en el formato esperado
                row.extend(balls_and_hands["rhand"])
                row.extend(balls_and_hands["lhand"])
                row.extend(balls_and_hands["balls"].flatten())
                annotations.append(row)
                count += 1 # Hasta aqui hay que indentar para usar el intercalador de frames para los videos de 60 fps (y sacar solo 30)
            success, image = vidcap.read()
            frame_idx += 1 # descomentar tambien al samplear a menos fps
        vidcap.release()
