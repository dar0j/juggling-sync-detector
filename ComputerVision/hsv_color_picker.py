...
path = '/home/dar0j/Documentos/2025/intro trabajo titulo el E/PROJECT/Datasets/5b/5_(6x,4x)_10.mp4'#5_(6x,4x)_12.mp4' # 0 for webcam
...
while True:
    ...
    # Resize output images to fit window size
    width = 360
    height = 640
    img_out_small = cv2.resize(img, (width, height))
    mask_small = cv2.resize(mask, (width, height))

    #show the image and wait
    cv2.imshow('img', img_out_small)
    cv2.imshow('image', mask_small)
    cv2.moveWindow('img', 2000, 20)
    cv2.moveWindow('image', 2000 + width + 20, 20)
    k=cv2.waitKey(100)
    if k==27: break
