...
if len(sys.argv) != 2:
    print("Usage: python trainpatternmodel.py <num_balls>")
    print("Example: python trainpatternmodel.py 3")
    print("Supported num_balls: 3, 4, 5, 6")
    sys.exit(1)

num_balls = int(sys.argv[1])
if num_balls not in [3, 4, 5, 6]:
    print("Error: num_balls must be 3, 4, 5, or 6")
    sys.exit(1)

# Load data and set up model filename
filename = f"{num_balls}balls"
seqfps = 30
gridmodel = 128 # 64 or 128
pdl = PatternDataLoader(filename, length=seqfps)
saveModelFilename = f"{num_balls}b_{seqfps}_{gridmodel}_model.h5"
...
metrics_result = model.evaluate(pdl.testx, pdl.testy)
print("Testset Accuracy: %.1f" % (metrics_result[1] * 100))
...
# Optional: Visualize a random sample from validation set (uncomment if needed)
